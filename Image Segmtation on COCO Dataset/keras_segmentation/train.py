...
def train( model  , 
	...
	if not validate:
		for ep in range( epochs ):
			...
			model.fit_generator( train_gen , steps_per_epoch  , epochs=1 , use_multiprocessing = True ,workers=2)
			if not checkpoints_path is None:
		...
		for ep in range( epochs ):
			print("Starting Epoch " , ep )
			model.fit_generator( train_gen , steps_per_epoch  , validation_data=val_gen , validation_steps=200 ,  epochs=1 , use_multiprocessing = True ,workers=2)
			if not checkpoints_path is None:
				model.save_weights( checkpoints_path + "." + str( ep )  )
				print("saved " , checkpoints_path + ".model." + str( ep ) )
			print("Finished Epoch" , ep )
